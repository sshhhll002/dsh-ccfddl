...
import { DAY_MS, HOUR_MS, SOURCE_URLS, derive, fetchText, parseYaml, targetOf } from './data.ts'
import type { Row, Settings } from './data.ts'
...
interface RemoteFailureShape { code: string; message: string }
type RemoteResult<T> = { ok: true; value: T } | { ok: false; error: RemoteFailureShape }
/** 服务器与浏览器两个通道共用的拉取结果形状。 */
interface FetchResult { ok: boolean; source: string; error: string; text: string }
type RemoteFetchFn = () => Promise<RemoteResult<FetchResult>>
...
function loadSettings(): Settings {
  ...
  try {
    ...
    if (text !== null) raw = JSON.parse(text) as Partial<Settings>
  } catch {
  ...
  if (typeof s.autoRefreshH !== 'number' || [0, 4, 8, 12, 24].indexOf(s.autoRefreshH) < 0) s.autoRefreshH = DEFAULT_SETTINGS.autoRefreshH
...
function fmtCount(ms: number, withSec: boolean): string {
  if (ms < 0) return '已截稿'
...
const shortDate = (s: string | null): string => (s !== null && s.length >= 10 ? s.slice(5, 10) : '')

/** 红绿灯紧急度:红 <7 天,橙 7–30 天,绿 >30 天。仅返回颜色修饰类。 */
function urgency(days: number): { cls: string; tip: string } {
  if (days < 0) return { cls: 'ccfddl-c-past', tip: '已截稿' }
  if (days < 7) return { cls: 'ccfddl-c-urg', tip: '红色:距截稿不足 7 天' }
  if (days < 30) return { cls: 'ccfddl-c-near', tip: '橙色:距截稿 7–30 天' }
...
function RadioGroup(p: RadioGroupProps): React.ReactElement {
  return React.createElement('div', { className: 'ccfddl-opts' },
    p.options.map((o) => React.createElement('label', { className: 'ccfddl-opt', key: String(o[0]) },
      React.createElement('input', { type: 'radio', name: p.name, checked: p.value === o[0], onChange: () => p.onChange(o[0]) }),
...
function Toggle(p: ToggleProps): React.ReactElement {
  return React.createElement('label', { className: 'ccfddl-opt' },
    React.createElement('input', { type: 'checkbox', checked: p.checked, onChange: (e) => p.onChange(e.target.checked) }),
...
function CcfIcon(): React.ReactElement {
  return React.createElement('svg', { width: 14, height: 14, viewBox: '0 0 14 14', fill: 'none', 'aria-hidden': true },
    ...
    React.createElement('path', { fill: 'currentColor', d: 'M6.55 2.6h0.9v4.4h-0.9z' }),
    React.createElement('path', { fill: 'currentColor', d: 'M7 6.55h4.4v0.9H7z' }),
  )
}

function CcfDdlWidget(props: { wide: boolean; remoteFetch: RemoteFetchFn | undefined }): React.ReactElement {
  const { wide, remoteFetch } = props
  const [settings, setSettings] = React.useState<Settings>(loadSettings)
  const [corpus, setCorpus] = React.useState<unknown[]>([])
  ...
  const doRefresh = async (): Promise<void> => {
    ...
    let usedUrl = SOURCE_URLS[0] as string
    ...
    if (raw === null) {
      for (const url of SOURCE_URLS) {
        ...
        if ('text' in result) { raw = result.text; usedUrl = url; break }
  ...
  if (!wide) {
    return React.createElement('div', { className: 'ccfddl-rail' },
      React.createElement('button', { className: 'ccfddl-rail-btn', title: 'CCF DDL 倒计时', 'aria-label': 'CCF DDL 倒计时', onClick: toggleCollapsed },
        React.createElement(CcfIcon),
      ),
    )
  }
  ...
  const { rows, catalog } = React.useMemo(
    () => derive(corpus, settings, now),
    [corpus, settings, now],
  )
  ...
  let minYear: string | number = ''
  for (const r of rows) {
    if (r.past) continue
    const k = targetOf(r, now)
    if (k > now && (minKey === null || k < minKey)) { minKey = k; minTitle = r.title; minYear = r.next.year }
  }

  if (collapsed) {
    const minDays = minKey !== null ? Math.floor((minKey - now) / DAY_MS) : -1
    const minUrg = urgency(minDays)
    return React.createElement('div', { className: 'ccfddl-root' },
      React.createElement('button', {
        ...
        React.createElement(CcfIcon),
        React.createElement('span', { className: 'ccfddl-bar-title' }, 'CCF DDL'),
        minTitle !== null && minKey !== null && phase === 'ready'
          ? React.createElement('span', { className: 'ccfddl-bar-name' }, minTitle + ' ' + minYear)
          : null,
        minKey !== null && phase === 'ready'
  ...
  const rowEl = (r: Row): React.ReactElement => {
    ...
    return React.createElement('div', { className: 'ccfddl-row', key: r.title, title: r.description || undefined, onClick: () => setExpanded(open ? null : r.title) },
      React.createElement('div', { className: 'ccfddl-row-main' },
        React.createElement('span', { className: 'ccfddl-rank ccfddl-rank-' + r.rank.ccf, title: 'CCF ' + r.rank.ccf + ' 类' }, r.rank.ccf),
        React.createElement('div', { className: 'ccfddl-row-title' },
          React.createElement('div', { className: 'ccfddl-row-name' }, r.title + ' ' + r.next.year),
          sub.length > 0 ? React.createElement('div', { className: 'ccfddl-row-sub' }, sub.join(' ▸ ') + (r.next.timezone ? ' (' + r.next.timezone + ')' : '')) : null,
        ),
        React.createElement('span', { className: 'ccfddl-count ' + u.cls, title: u.tip }, fmtCount(ms, r.title === minTitle)),
      ),
      open ? React.createElement('div', { className: 'ccfddl-detail' },
        isPast ? React.createElement('div', null, '已截稿') : null,
        React.createElement('div', null, '等级: CCF ' + r.rank.ccf + (r.rank.core ? ' · CORE ' + r.rank.core : '') + (r.rank.thcpl ? ' · THCPL ' + r.rank.thcpl : '')),
        r.next.abstractText ? React.createElement('div', null, 'Abstract: ' + r.next.abstractText + ' (' + r.next.timezone + ')') : null,
        r.next.paperText ? React.createElement('div', null, 'Paper: ' + r.next.paperText + ' (' + r.next.timezone + ')') : null,
        r.next.date ? React.createElement('div', null, '会议时间: ' + r.next.date) : null,
        r.next.place ? React.createElement('div', null, '地点: ' + r.next.place) : null,
        r.next.link ? React.createElement('a', { className: 'ccfddl-link', href: r.next.link, target: '_blank', rel: 'noopener noreferrer', onClick: (e: React.MouseEvent) => e.stopPropagation() }, '官网 ↗') : null,
      ) : null,
    )
  }

  const headerEl = React.createElement('div', { className: 'ccfddl-header' },
    React.createElement(CcfIcon),
    React.createElement('span', { className: 'ccfddl-header-title' }, 'CCF DDL'),
    React.createElement('div', { className: 'ccfddl-header-btns' },
      React.createElement('button', { className: 'ccfddl-ibtn' + (meta !== null && meta.stale ? ' stale' : ''), title: meta !== null && meta.stale ? '数据已过期,点击重试' : '刷新数据', disabled: refreshing, onClick: () => void doRefresh() }, refreshing ? '…' : '↻'),
      React.createElement('button', { className: 'ccfddl-ibtn' + (panel ? ' on' : ''), title: '设置', onClick: () => setPanel(!panel) }, '⚙'),
      React.createElement('button', { className: 'ccfddl-ibtn', title: '最小化到一行', onClick: toggleCollapsed }, '—'),
    ),
  )

  let bodyEl: React.ReactElement | null = null
  if (panel) {
    const TABS: Array<[string, string]> = [['track', '跟踪'], ['filter', '筛选'], ['update', '更新'], ['about', '关于']]
    const trackTabEl = (): React.ReactElement => {
      ...
      return React.createElement('div', null,
        React.createElement('input', { className: 'ccfddl-search', type: 'text', placeholder: '搜索会议…', value: search, onChange: (e) => setSearch(e.target.value) }),
        React.createElement('div', { className: 'ccfddl-chips' },
          ['all'].concat(subs).map((c) => React.createElement('button', { key: c, className: 'ccfddl-chip' + (subChip === c ? ' active' : ''), onClick: () => setSubChip(c) }, c === 'all' ? '全部' : c)),
        ),
        React.createElement('div', { className: 'ccfddl-actions' },
          React.createElement('button', { className: 'ccfddl-action', disabled: catalog.length === 0, onClick: () => change({ tracked: catalog.map((c) => c.title) }) }, '全选'),
          React.createElement('button', { className: 'ccfddl-action', disabled: catalog.length === 0, onClick: () => change({ tracked: [] }) }, '清空'),
          React.createElement('span', { className: 'ccfddl-note' }, '已选 ' + tracked.length + ' 个'),
        ),
        React.createElement('div', { className: 'ccfddl-checks' },
          catalog.length === 0
            ? React.createElement('div', { className: 'ccfddl-note' }, '数据加载中…')
            : visible.map((c) => React.createElement('label', { className: 'ccfddl-check', key: c.title },
                React.createElement('input', { type: 'checkbox', checked: tracked.indexOf(c.title) >= 0, onChange: () => {
                ...
                React.createElement('span', { className: 'ccfddl-rank ccfddl-rank-' + c.rank }, c.rank),
                React.createElement('span', { className: 'ccfddl-check-name' }, c.title),
                React.createElement('span', { className: 'ccfddl-chip' }, c.sub),
              )),
        ),
      )
    }
    const filterTabEl = (): React.ReactElement => React.createElement('div', null,
      React.createElement('div', { className: 'ccfddl-section-title' }, '时间窗口'),
      React.createElement(RadioGroup, { name: 'win', value: settings.windowDays, options: [[30, '30 天'], [60, '60 天'], [90, '90 天'], [180, '180 天'], [0, '全部']], onChange: (v) => change({ windowDays: Number(v) }) }),
      React.createElement('div', { className: 'ccfddl-section-title' }, '排序方式'),
      React.createElement(RadioGroup, { name: 'sort', value: settings.sortBy, options: [['deadline', '按截稿'], ['rank', '按等级'], ['title', '按名称']], onChange: (v) => change({ sortBy: v as Settings['sortBy'] }) }),
      React.createElement('div', { className: 'ccfddl-section-title' }, '等级筛选'),
      React.createElement('div', { className: 'ccfddl-opts' },
        ['A', 'B', 'C', 'N'].map((r) => React.createElement('label', { className: 'ccfddl-opt', key: r },
          React.createElement('input', { type: 'checkbox', checked: settings.rankAllow.indexOf(r) >= 0, onChange: (e) => {
          ...
          React.createElement('span', null, r + ' 类'),
        )),
      ),
      React.createElement('div', { className: 'ccfddl-opts' },
        React.createElement(Toggle, { checked: settings.showAbstract, label: '显示 abstract 截稿行', onChange: (v) => change({ showAbstract: v }) }),
        React.createElement(Toggle, { checked: settings.showPast, label: '显示已过期会议', onChange: (v) => change({ showPast: v }) }),
      ),
    )
    const updateTabEl = (): React.ReactElement => React.createElement('div', null,
      React.createElement('div', { className: 'ccfddl-note' }, meta !== null && meta.updatedAt > 0 ? '上次成功更新: ' + fmtClock(meta.updatedAt) + ' (本地时间)' : '尚未成功更新'),
      meta !== null && meta.stale ? React.createElement('div', { className: 'ccfddl-error' }, '⚠ ' + meta.lastError) : null,
      React.createElement('div', { className: 'ccfddl-section-title' }, '自动刷新间隔'),
      React.createElement(RadioGroup, { name: 'auto', value: settings.autoRefreshH, options: [[0, '关'], [4, '4 小时'], [8, '8 小时'], [12, '12 小时'], [24, '24 小时']], onChange: (v) => change({ autoRefreshH: Number(v) }) }),
    )
    const aboutTabEl = (): React.ReactElement => React.createElement('div', null,
      React.createElement('div', { className: 'ccfddl-section-title' }, '数据源'),
      React.createElement('div', { className: 'ccfddl-note' }, meta?.sourceUrl ?? ''),
      React.createElement('a', { className: 'ccfddl-link', href: 'https://github.com/ccfddl/ccf-deadlines', target: '_blank', rel: 'noopener noreferrer' }, 'ccfddl/ccf-deadlines ↗'),
      React.createElement('div', { className: 'ccfddl-note' }, '数据按 MIT 许可取自该项目'),
      React.createElement('div', { className: 'ccfddl-section-title' }, '插件'),
      React.createElement('div', { className: 'ccfddl-note' }, 'CCF DDL Countdown v' + VERSION + ' · 设置保存在浏览器本地'),
      React.createElement('div', { className: 'ccfddl-note' }, '非官方插件,与 CCF 及 ccfddl 无隶属关系'),
    )
    bodyEl = React.createElement('div', { className: 'ccfddl-panel' },
      React.createElement('div', { className: 'ccfddl-tabs' },
        TABS.map((t) => React.createElement('button', { key: t[0], className: 'ccfddl-tab' + (tab === t[0] ? ' active' : ''), onClick: () => setTab(t[0]) }, t[1])),
      ),
      React.createElement('div', { className: 'ccfddl-panel-body' },
        tab === 'track' ? trackTabEl() : null,
        tab === 'filter' ? filterTabEl() : null,
        tab === 'update' ? updateTabEl() : null,
        tab === 'about' ? aboutTabEl() : null,
      ),
    )
  } else if (phase === 'loading') {
    bodyEl = React.createElement('div', { className: 'ccfddl-list' },
      React.createElement('div', { className: 'ccfddl-skel' }),
      React.createElement('div', { className: 'ccfddl-skel' }),
      React.createElement('div', { className: 'ccfddl-skel' }),
    )
  } else if (phase === 'error') {
    bodyEl = React.createElement('div', { className: 'ccfddl-error' },
      React.createElement('div', null, '⚠ 数据加载失败'),
      React.createElement('div', { className: 'ccfddl-note' }, meta?.lastError ?? ''),
      React.createElement('button', { className: 'ccfddl-action primary', onClick: () => void doRefresh() }, '重试'),
    )
  } else if (empty) {
    bodyEl = React.createElement('div', { className: 'ccfddl-empty' },
      React.createElement('div', { className: 'ccfddl-empty-icon' }, '🕐'),
      React.createElement('div', null, '没有符合条件的会议'),
      React.createElement('button', { className: 'ccfddl-action', onClick: () => { setPanel(true); setTab('filter') } }, '调整筛选'),
    )
  } else {
...
export async function apply(ctx: Context): Promise<void> {
  const slots = ctx.get('slots') as {
    register: (options: object, render: (props: { wide: boolean }) => React.ReactElement) => () => void
    inject: (key: string, callback: () => () => void) => () => void
  } | undefined
  if (slots === undefined) return

  // 挂载服务器数据通道;重复挂载或远端缺失时静默,组件自动降级浏览器直连。
  let remoteFetch: RemoteFetchFn | undefined
  const remote = ctx.get('remote') as { $mount: (contribution: unknown) => Promise<unknown> } | undefined
  if (remote !== undefined) {
    try {
    ...
    const channel = ctx.get('remote.ccfddl.data') as { fetchData: () => Promise<RemoteResult<FetchResult>> } | undefined
    if (channel !== undefined) remoteFetch = () => channel.fetchData()
  }
  ...
  slots.inject('sidebar.footer.action', () => slots.register(
    { name: 'sidebar.footer.action', id: 'ccf-ddl', order: 0, label: () => 'CCF DDL 倒计时' },
    (props: { wide: boolean }) => React.createElement(CcfDdlWidget, { wide: props.wide, remoteFetch }),
  ))
}
