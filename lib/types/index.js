 ...
 */
export async function fetchCorpus(urls, fetcher) {
    const failures = [];
    for (const url of urls) {
        try {
            ...
            if (text.length > 100)
                return { ok: true, source: url, error: '', text };
...
let CcfddlData = (() => {
    ...
    return class CcfddlData extends _classSuper {
        static {
            ...
            __esDecorate(this, null, _fetchData_decorators, { kind: "method", name: "fetchData", static: false, private: false, access: { has: obj => "fetchData" in obj, get: obj => obj.fetchData }, metadata: _metadata }, null, _instanceExtraInitializers);
        ...
        async fetchData() {
            ...
            if (result.ok)
                this.cache = { text: result.text, source: result.source, at: Date.now() };
            return result;
        }
    };
})();
